...
const FoodListingVisibility: React.FC<Props> = ({ listing, onStatusChange }) => {
  ...
  const isDonor     = user?.role === 'donor' && (user as any)._id === (listing as any).user;
  const isRecipient = user?.role === 'recipient';
  const hasCoords   = !!(listing.location?.coordinates);
  const statusCfg   = getStatusConfig(listing);
  const isRequested = (listing as any).status === 'requested';
    ...
    <div style={{ background:'#fafafa', borderRadius:14, border:'1px solid #f1f5f9', overflow:'hidden', opacity:0.6, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      <button onClick={() => setCollapsed(c=>!c)}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <p style={{ fontSize:'0.7rem', color:'#94a3b8' }}>{listing.servings} servings · Expired</p>
        </div>
      ...
      <div style={{ padding:'16px 18px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:7, marginBottom:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:'0.78rem', color:'#64748b' }}>
            ...
            <CountdownTimer expiryDate={new Date(new Date(listing.createdAt).getTime() + parseFloat(listing.bestBefore) * 3_600_000)}/>
          </div>
